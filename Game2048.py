...
class Game2048:
    ...
    def create_widgets(self):
        self.score_label = tk.Label(
            self.window, text="Score: 0", font=("Helvetica", 16)
        )
        self.score_label.pack(pady=10)

        self.canvas = tk.Canvas(
            self.window, width=self.canvas_width, height=self.canvas_height, bg=BG_COLOR
        )
        self.canvas.pack()
    ...
    def merge_up(self):
        ...
        for row in transposed:
            ...
            merged_board.append(merged_row)
        self.board = [
            [merged_board[j][i] for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)
        ]
